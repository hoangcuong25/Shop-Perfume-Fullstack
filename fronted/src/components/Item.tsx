...
const Item = ({
    ...
    return (
        <Link to={`/product/${id}`} onClick={() => window.scrollTo(0, 0)}>
            <div className="flex flex-col justify-center items-center hover:scale-105 transition-all duration-500 mt-5 md:mx-3 relative group">
                <img src={image} alt="" className="md:size-[185px] size-[130px]" />
                <h3 className="font-semibold text-base md:text-lg ">{brand}</h3>
                <h4 className="italic w-32 text-center text-xs md:text-base text-gray-900">{name}</h4>
